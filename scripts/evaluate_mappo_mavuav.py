...
def main():
    ...
    if data.get("environment_profile") != args.profile: raise RuntimeError(f"incompatible MAPPO checkpoint environment profile: {data.get('environment_profile')!r} (expected {args.profile!r})")
    actor = GaussianActor(hidden_dim=int(data["config"]["hidden_dim"])); actor.load_state_dict(data["actor"]); actor.eval()
    for mode in ("nearest", "mav_priority"): print(mode, summarize_records(evaluate_actor(actor, args.env_config, args.episodes, mode, args.profile)))
