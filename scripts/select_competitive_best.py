...
from uav_combat.mappo.trainer import CHECKPOINT_VERSION,competitive_score,evaluate_competitive_match,evaluate_competitive_match_parallel,resolve_device
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--output-dir",default="outputs/mappo_v6");p.add_argument("--env-config",default="configs/homogeneous_1v1.yaml");p.add_argument("--episodes",type=int,default=300);p.add_argument("--device",default="auto");p.add_argument("--env-workers",type=int,default=4);args=p.parse_args()
    ...
    best_a,best_b,best_c=load(destination,device);best_eval=evaluate_competitive_match_parallel(best_a,best_b,args.env_config,args.episodes,device,num_env_workers=args.env_workers,seed=best_c["config"]["experiment"]["seed"]+300000)
    best_overall = best_eval["overall"]
    summary.update({"formal_best_score":json.loads(rows[0]["formal_score"]),"formal_best_checkpoint":str(destination.relative_to(output)),"initial_formal_evaluation":evaluations.get(initial_key),"competitive_best_formal_evaluation":best_eval,"final_formal_evaluation":evaluations.get(final_key),"formal_policy_a_kill_rate":best_overall["policy_a_kill_rate"],"formal_policy_b_kill_rate":best_overall["policy_b_kill_rate"],"formal_policy_a_boundary_loss_rate":best_overall["policy_a_boundary_loss_rate"],"formal_policy_b_boundary_loss_rate":best_overall["policy_b_boundary_loss_rate"],"formal_policy_a_role_kill_gap":best_overall["policy_a_role_kill_gap"],"formal_policy_b_role_kill_gap":best_overall["policy_b_role_kill_gap"],"formal_worst_scenario_combat_decisive_rate":best_overall["worst_scenario_combat_decisive_rate"],"candidate_selection_seconds":time.perf_counter()-t_start})
