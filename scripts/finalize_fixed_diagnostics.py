...
def main():
    ...
    final=torch.load(args.final_checkpoint,map_location=device,weights_only=False); final["fixed_gate_result"]=gate; final["training_stage"]="fixed_gate"; final["active_side"]="red"; torch.save(final,args.final_checkpoint)
    ...
    result={"checkpoint":"fixed_best","actor":"red","opponent":"pursuit","scenario":"all","seed_set":"seedset0","episodes":args.episodes,"environment_steps_observed":steps,"gate_win_rate":evaluation["overall"]["win_rate"],"gate_pass":gate,"reward_component_mean_per_step":{key:value/steps for key,value in rewards.items()},"actions":{name:{"mean":float(np.mean(values)),"std":float(np.std(values)),"min":float(np.min(values)),"max":float(np.max(values))} for name,values in action_values.items()},"mean_control_diagnostics":{key:float(np.mean(values)) for key,values in control_values.items()},"control_saturation_rates":{key:float(np.mean(values)) for key,values in saturation.items()},"attack_funnel":{key:evaluation["overall"][key] for key in ("within_4000_rate","attack_distance_entry_rate","ata_gate_rate","aa_gate_rate","attack_envelope_entry_rate","attack_to_kill_conversion_rate","kill_rate")}}
    ...
    for section in ("reward_component_mean_per_step","mean_control_diagnostics","control_saturation_rates","attack_funnel"): flat.update({f"{section}.{key}":value for key,value in result[section].items()})
    ...
    for filename,title,data in (("attack_funnel.png","Attack funnel (project diagnostics)",result["attack_funnel"]),("control_saturation.png","Controller saturation rates",result["control_saturation_rates"])):
