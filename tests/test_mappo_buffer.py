...
def test_buffer_shapes_and_manual_gae_done_mask():
    ...
    for done in (False, True, False):
        buffer.add(np.zeros((1, 2, 14)), np.zeros((1, 14)), np.zeros((1, 2, 3)), np.zeros((1, 2)), np.ones((1, 2)), np.zeros((1, 2)), np.array([done]))
    buffer.compute_returns_and_advantages(np.zeros((1, 2)), gamma=1.0, gae_lambda=1.0)
    assert buffer.observations.shape == (3, 1, 2, 14)
...
def test_buffer_rejects_wrong_dimensions():
    ...
    with pytest.raises(ValueError):
        buffer.add(np.zeros((2, 14)), np.zeros((1, 14)), np.zeros((1, 2, 3)), np.zeros((1, 2)), np.zeros((1, 2)), np.zeros((1, 2)), np.zeros(1, dtype=bool))
