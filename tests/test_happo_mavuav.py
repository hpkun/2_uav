...
def test_v35_observation_state_contract_and_vanilla_parameter_count():
    trainer = HAPPOTrainer(config={"num_envs": 1, "rollout_steps": 1, "hidden_dim": 128})
    assert ENVIRONMENT_VERSION == "heterogeneous_mavuav_4v4_v3_5"
...
def test_happo_rejects_v34_checkpoint_contract(tmp_path):
    source = HAPPOTrainer(config={"num_envs": 1, "rollout_steps": 1, "hidden_dim": 8})
    checkpoint = tmp_path / "v34.pt"
    source.save_checkpoint(checkpoint); source.close()
    payload = torch.load(checkpoint, map_location="cpu", weights_only=False)
    payload["environment_version"] = "heterogeneous_mavuav_4v4_v3_4"
    torch.save(payload, checkpoint)
    target = HAPPOTrainer(config={"num_envs": 1, "rollout_steps": 1, "hidden_dim": 8})
    with pytest.raises(RuntimeError, match="incompatible checkpoint contract"):
        target.load_checkpoint(checkpoint)
