...
The centralized state-only critic parses the exact 117D state as eight 10D aircraft blocks, 32 cross-team attack-streak values, four Blue-kill flags, and normalized episode progress. A `GRUCell(117,128)` supplies temporal context before a single 4-head attention layer over eight aircraft tokens plus one always-valid context token. Dead aircraft are masked. Masked pooling feeds `128 -> 256 -> 128 -> 1`, producing team `V(s)`.
