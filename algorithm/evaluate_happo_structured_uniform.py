"""Deterministically evaluate a HAPPO Structured Uniform checkpoint."""
...
def main() -> None:
    ...
    if variant != "structured_uniform" or not isinstance(architecture, dict):
        raise RuntimeError(
            "incompatible actor architecture: structured_uniform evaluator requires "
            "a structured_uniform checkpoint"
        )
    expected_keys = {"entity_dim", "role_dim", "fusion_hidden_dim", "action_dim"}
    if set(architecture) != expected_keys:
        raise RuntimeError("incompatible structured_uniform actor architecture metadata")
    actors = StructuredUniformIndependentActors(
        entity_dim=int(architecture["entity_dim"]),
        role_dim=int(architecture["role_dim"]),
    ...
    csv_path = checkpoint.parent / f"evaluation_structured_uniform_{label}.csv"
    with csv_path.open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    summary_path = checkpoint.parent / f"evaluation_structured_uniform_{label}_summary.json"
    summary = {
        "algorithm": "happo_structured_uniform",
