...
from ..mappo.vector_env_4v3_v11 import make_combat_vector_env_4v3_v11
from ..mappo.vector_env_4v3_v12 import make_combat_vector_env_4v3_v12
    ...
    *,
    ...
    try:
        ...
        for start in range(0, len(seed_list), batch_capacity):
            ...
            env_contract = yaml.safe_load(Path(env_config).read_text(encoding="utf-8"))
            reward_contract_version = env_contract.get("combat", {}).get("reward_contract_version")
            if reward_contract_version == "v12_soft_boundary_combat_aligned":
                vec_factory = make_combat_vector_env_4v3_v12
            elif reward_contract_version == "v11_target_lock_support_cue":
                vec_factory = make_combat_vector_env_4v3_v11
            else:
                vec_factory = make_combat_vector_env_4v3
            vec = vec_factory(env_config, len(batch_seeds), workers, seed=0)
            try:
...
def evaluate_rule_vs_rule_4v3(
    ...
    *,
    ...
    if config.get("combat", {}).get("reward_contract_version") == "v12_soft_boundary_combat_aligned":
        if red_policy not in ("rule", "random"):
            raise ValueError("v12 red_policy must be 'rule' or 'random'")
        args = [(str(env_config), int(item_seed), red_policy) for item_seed in seed_list]
        t0 = time.perf_counter()
        if int(workers) <= 1 or len(args) <= 1:
            records = [_run_v12_baseline_episode(*item) for item in args]
        else:
            with ProcessPoolExecutor(max_workers=min(int(workers), len(args))) as executor:
                records = list(executor.map(_run_v12_baseline_episode, [item[0] for item in args], [item[1] for item in args], [item[2] for item in args]))
        records.sort(key=lambda record: int(record["episode_seed"]))
...
def _run_v12_baseline_episode(env_config: str, episode_seed: int, red_policy: str = "rule") -> dict[str, Any]:
    ...
    while not done:
        if red_policy == "rule":
            ...
            raise ValueError(f"unsupported v12 red policy: {red_policy!r}")
        _, _, _, _, done, _, info = env.step(actions)
    record = deepcopy(info["episode_summary"])
    record["episode_seed"] = int(episode_seed)
    record["red_policy"] = red_policy
