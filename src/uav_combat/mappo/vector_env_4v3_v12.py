"""Vector adapter for the v12 soft-boundary 4v3 environment."""
...
from ..scenario_4v3_v12 import BLUE_IDS_V12, RED_IDS_V12


class VectorStepResult4v3V12(NamedTuple):
...
def _actions_for_env(actions: np.ndarray) -> dict[str, np.ndarray]:
    ...
    if values.shape != (RED_TEAM_SIZE_V12, 3):
        raise ValueError(f"v12 red actions must have shape (4, 3), got {values.shape}")
    return {aid: np.asarray(values[index], dtype=np.float32) for index, aid in enumerate(RED_IDS_V12)}


def _hold(env: FunctionalHeterogeneous4v3V12SoftBoundaryCombatAlignedEnv):
    obs, state, masks = env._observations()
    return obs, state, masks, 0.0, False, False, {
        "reward_components": {key: 0.0 for key in REWARD_COMPONENT_KEYS_V12},
        "reward_groups": {key: 0.0 for key in ("mission", "combat_evt", "support_evt", "half_lock_evt", "boundary_evt", "dense")},
        "raw_dense_reward": 0.0,
        "episode_summary": None,
    }


def _pack(results: list[tuple[Any, ...]]) -> VectorStepResult4v3V12:
    obs, states, masks, rewards, terms, truncs, infos = zip(*results)
    components = np.zeros((len(results), len(REWARD_COMPONENT_KEYS_V12)), dtype=np.float32)
    summaries: list[dict[str, Any] | None] = []
    valid = np.zeros(len(results), dtype=bool)
    for row, info in enumerate(infos):
        for column, key in enumerate(REWARD_COMPONENT_KEYS_V12):
            components[row, column] = float(info.get("reward_components", {}).get(key, 0.0))
        summary = info.get("episode_summary")
        summaries.append(summary)
        valid[row] = summary is not None
    return VectorStepResult4v3V12(
...
class LocalCombatVectorEnv4v3V12:
    def __init__(self, config_path: str | Path, num_envs: int, seed: int = 0) -> None:
        ...
        self.seed = int(seed)
        self.envs = [FunctionalHeterogeneous4v3V12SoftBoundaryCombatAlignedEnv(config_path) for _ in range(self.num_envs)]
        self.reset()
    ...
    def reset_at(self, index: int | np.ndarray, seed: int | list[int] | np.ndarray):
        ...
        values = [self.envs[int(item)].reset(int(item_seed)) for item, item_seed in zip(indices, seeds)]
    ...
    def step(self, actions: np.ndarray) -> VectorStepResult4v3V12:
        values = np.asarray(actions)
        if values.shape != (self.num_envs, RED_TEAM_SIZE_V12, 3):
            raise ValueError(f"v12 batched actions must have shape ({self.num_envs}, 4, 3)")
        return _pack([
    ...
    def policy_modes(self) -> dict[str, list[str]]:
        ...
        return {
            "red": [config.get("red_rule_policy", {}).get("mode", "v12_rule")] * self.num_envs,
            "blue": [config.get("blue_rule_policy", {}).get("mode", "v12_rule")] * self.num_envs,
        }
...
def _worker(conn: multiprocessing.connection.Connection, config_path: str, envs_per_worker: int, worker_index: int, seed: int) -> None:
    try:
        envs = [FunctionalHeterogeneous4v3V12SoftBoundaryCombatAlignedEnv(config_path) for _ in range(envs_per_worker)]
        ...
        while True:
            ...
            if command == "reset":
                ...
                conn.send({
                    "red": [config.get("red_rule_policy", {}).get("mode", "v12_rule")] * envs_per_worker,
                    "blue": [config.get("blue_rule_policy", {}).get("mode", "v12_rule")] * envs_per_worker,
                })
...
class SubprocessCombatVectorEnv4v3V12:
    def __init__(self, config_path: str | Path, num_envs: int, num_workers: int, seed: int = 0) -> None:
        if int(num_workers) <= 0 or int(num_workers) > int(num_envs) or int(num_envs) % int(num_workers) != 0:
            raise ValueError("v12 num_workers must divide num_envs and be positive")
        self.config_path = str(config_path)
        self.num_envs = int(num_envs)
    ...
    def step(self, actions: np.ndarray) -> VectorStepResult4v3V12:
        ...
        parts = self._recv()
        return VectorStepResult4v3V12(
            *(np.concatenate([getattr(part, field) for part in parts], axis=0) if field != "episode_summaries" else [summary for part in parts for summary in part.episode_summaries] for field in VectorStepResult4v3V12._fields)
        )
...
def make_combat_vector_env_4v3_v12(config_path: str | Path, num_envs: int, num_env_workers: int = 0, seed: int = 0):
    if int(num_env_workers) <= 0:
        return LocalCombatVectorEnv4v3V12(config_path, num_envs, seed)
    return SubprocessCombatVectorEnv4v3V12(config_path, num_envs, num_env_workers, seed)


__all__ = [
    "BLUE_TEAM_SIZE_V12", "GS_DIM_V12", "OBS_DIM_V12", "RED_TEAM_SIZE_V12",
    "REWARD_COMPONENT_KEYS_V12", "VectorStepResult4v3V12", "make_combat_vector_env_4v3_v12",
]
